...
Documented all-caps acronym shorthands such as `RKE`, `QTK`, and `DDD` are valid invocation forms for the corresponding skills.
...
- [doc-driven-development](./skills/engineering/doc-driven-development) (DDD): turn epic or end-state product truth into feature contracts, work packages, and acceptance artifacts before coding, then recommend publishing stable packages to the chosen tracker or task surface.
- [repo-dissection](./skills/engineering/repo-dissection) (RD): dissect inherited or unclear repositories and turn assumptions into verified documented understanding.
...
- [repo-knowledge-engineering](./skills/engineering/repo-knowledge-engineering) (RKE): establish and maintain the repository knowledge framework, canonical product truth, reading order, plans, decisions, glossary, and validation evidence.
- [local-handoff](./skills/engineering/local-handoff) (LH): write a dated local handoff so the next session can resume safely.
- [local-pickup](./skills/engineering/local-pickup) (LP): resume from a local handoff and rebuild trustworthy context before editing.
...
- [pandoc-converter](./skills/automation/pandoc-converter) (PC): run Pandoc conversions with predictable defaults while still allowing custom flags.
...
- [agenda-generator](./skills/content/agenda-generator) (AG): draft lean or formal meeting agendas from simple prompts or richer context.
- [linkedin-short-post-drafter](./skills/content/linkedin-short-post-drafter) (LSPD): write short LinkedIn-style posts for updates, launches, events, and capability highlights.
- [long-form-post-drafter](./skills/content/long-form-post-drafter) (LFPD): build evidence-grounded long-form posts, launch articles, and blog-style content.
...
- [scheduling-assistant](./skills/content/scheduling-assistant) (SA): turn meeting requests into calendar-aware slot proposals and ready-to-send emails.
...
- [ai-initiative-deep-dive-and-scoping](./skills/delivery/ai-initiative-deep-dive-and-scoping) (AIDDS): pressure-test and scope AI initiatives that are ready for deeper validation.
- [clickup-project-plan-builder](./skills/delivery/clickup-project-plan-builder) (CPPB): turn project briefs into practical ClickUp structures, hierarchy, tags, and views.
...
- [project-packager](./skills/delivery/project-packager) (PP): turn an existing `PROJECT.md` into audience-specific or system-ready project outputs.
...
- [project-support](./skills/delivery/project-support) (PS): orient and validate real project work before a more specialized project skill takes over.
- [training-plan-writer](./skills/delivery/training-plan-writer) (TRPW): create paired customer-facing and facilitator-grade training plans from agreed scope.
...
- [source-derived-design-system-builder](./skills/design/source-derived-design-system-builder) (SDDSB): turn real visual references into a reusable design skill and persistent `DESIGN.md`.
...
- [knowledge-transfer-documentation-writer](./skills/documentation/knowledge-transfer-documentation-writer) (KTDW): write concise internal knowledge transfer docs from authoritative source material.
...
- [elevenlabs-ai-voice-gen](./skills/media/elevenlabs-ai-voice-gen) (EAVG): write and clean narration scripts for ElevenLabs voice generation.
- [remotion-explainer-video-production](./skills/media/remotion-explainer-video-production) (REVP): create Remotion explainer-video plans, timing layouts, overlays, and branded composition guidance.

### Meta

Location: [skills/meta](./skills/meta)

- [skill-eval-suite-writer](./skills/meta/skill-eval-suite-writer) (SESW): build evaluation suites, scenario matrices, and grader strategies for skills.
- [llm-instruction-fixer](./skills/meta/llm-instruction-fixer) (LLMIF): repair prompts, skills, and other LLM instruction artifacts from a review or fix brief.
- [llm-instruction-reviewer](./skills/meta/llm-instruction-reviewer) (LLMIR): inspect prompts and instruction artifacts for execution risks before publication or repair.
- [skill-finaliser](./skills/meta/skill-finaliser) (SF): turn draft or imported skills into clean, publishable skill packages.
...
- use `doc-driven-development` (`DDD`) to decompose that truth into feature contracts and work packages
- use `repo-dissection` (`RD`) first when the codebase truth is unclear
...
- use `repo-knowledge-engineering` (`RKE`) to keep the repository knowledge base aligned in the same slice after implementation or validation work lands
- use `local-handoff` (`LH`) when pausing
- use `local-pickup` (`LP`) when resuming

## Recommended Flows

### Inherited unclear repo

`repo-dissection -> repo-knowledge-engineering -> doc-driven-development -> query-to-knowledge -> tdd -> repo-knowledge-engineering -> local-handoff`

### Resumed implementation tranche

`local-pickup -> repo-knowledge-engineering -> doc-driven-development -> query-to-knowledge -> tdd -> repo-knowledge-engineering -> local-handoff`
