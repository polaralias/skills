...
description: Turn an epic, end-state product definition, or scoped outcome into implementation-ready feature docs, work packages, and acceptance artifacts before coding. Use when a user wants docs-first delivery, feature decomposition, issue-ready work breakdown, or behavior contracts that later drive TDD. Shorthand DDD.
...
Use this skill to turn product truth into implementation-ready documentation.

This skill sits between repository knowledge setup and implementation.
It takes a defined epic, product outcome, or end-state contract and breaks it into feature-level and task-level artifacts that can drive delivery.

The output is not vague planning prose.
The output is a bounded behavioral contract that downstream implementation can build against.
...
- Use this skill when the main job is to decompose a resolved outcome into implementation-ready units and acceptance artifacts.
...
### 6. Break features into work packages
...
Once the package set is stable, recommend publishing it to the user's issue tracker or local task surface so implementation can run against tracked work rather than chat-only artifacts.
