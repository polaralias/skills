...
For durable modes, `task check` verifies the configured executable identity with `okf-tasks --version` and runs the authoritative `validate --strict` command against the configured repository and bundle. It returns a structured envelope containing the adapter version, invoked argument vector, captured validation result, and exit status. It never invokes a shell.
