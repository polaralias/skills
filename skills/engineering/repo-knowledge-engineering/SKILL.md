...
- For standalone OKF HTML, require the pinned rendering runtimes and bundle data to be embedded so local review makes no runtime network requests. On Windows, allow the generator to clear inherited `Zone.Identifier` metadata only from the HTML output it has just written, preventing stale download warnings without altering unrelated files. For dense relationship graphs, preserve labelled semantic landmarks at overview scale, expand the connected map to suit a wide canvas without over-stretching it, and keep disconnected documents in a bounded reveal-on-hover shelf that does not control readable scale. Verify that a selected neighbourhood uses close framing and modest padding while keeping every direct node readable, visible, and non-overlapping.
