...
The viewer defaults to a Focus relationship explorer in Graph, with the selected record between readable incoming and outgoing record cards. Topology mode keeps the complete bundle available with bounded compact labels, class-colored borders, switchable layouts, and relationship labels around the active node. Kanban groups Tasks and Workstreams by lifecycle status, both graph modes share one detailed preview, and Documents provides a searchable near-full-page Markdown reader. The viewer surfaces effort and connection metrics, summarizes recorded time by workstream, and offers a Timeline plus through-date filtering for `timestamp`, `created`, `started`, and `finished`. Drift review compares selected timestamps across existing links and carries possible signals into Graph and Kanban. Treat every highlight as a possible review signal requiring semantic evidence, not proof that the older target is stale or a reconstruction of its earlier content. Generated pages use the pinned Apache-2.0 `@mdi/font` 7.4.47 webfont from Pictogrammers for interface icons.
