...
This skill is for disciplined repair, not reinvention. The job is to remove execution risk while keeping the artefact recognisably the same artefact.
...
1. Read both the artefact and the fix brief completely.
...
3. Contain unsafe trust-boundary, authority, data-access, and egress behaviour before lower-risk wording fixes.
...
- Reuse suggested rewrites only when they fit the artefact's voice and level of formality.
...
- multiple plausible repairs would lead to meaningfully different behaviour
